...
### Веб-версия: Alert.alert молча ничего не делает (2026-08-30)
`Alert.alert` не реализован в react-native-web — на вебе вызовы проходят без ошибки и без
диалога. В коде 27 мест (`grep -rn "Alert.alert" app components`): подтверждения удаления,
ошибки форм, выход из аккаунта. На Android всё работает, на вебе пользователь просто не
видит реакции.

**Как чинить:** заменить на `ConfirmModal`/`useConfirm` из дизайн-системы — они уже есть и
работают на обеих платформах (см. [[ui-modals]]). Заодно уйдёт расхождение с
[[rules-design-system]], где нативные примитивы и так не приветствуются.

**Ещё не проверено на вебе:** экраны целей, календари, создание цели, шэринг инвайта
(нативный `Share` на вебе тоже недоступен). Проверялся только вход.
