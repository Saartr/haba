...
### Перевыпустить ключи веб-приложения VK (2026-09-09)
Secure key и Service token приложения `54761714` были продиктованы в переписке, то есть лежат
в стенограмме сессии на диске. Отредактировать её задним числом нельзя — вопрос решается
перевыпуском ключей в консоли VK ID и заменой `VK_WEB_CLIENT_SECRET` / `VK_WEB_SERVICE_TOKEN`
в `.env` на сервере. Делать после того, как вход перестанут активно проверять.
