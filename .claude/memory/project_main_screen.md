...
- Иконка сервиса рядом с аватаром — берётся из `user.last_login_provider` (способ последнего входа), с фоллбэком на `tg_id`/`vk_id` для сессий до миграции; см. [[project_database]] (при слиянии аккаунтов оба ID заполнены одновременно, поэтому наличие `tg_id` само по себе не говорит, чем залогинились последний раз)
- `HabitCard` для `category === 'pullups'`: подзаголовок «Цель на сегодня» + значение «N подходов по N повторений» (тренировочный день) или «Отдых» (день не из `training_days`) — не «Шагов за сегодня»/`N/goal`, как у остальных категорий. Аватар-иллюстрация недовольная (`AngryAvatar`) только если сегодня тренировочный день и цель ещё не выполнена; день отдыха или выполненная тренировка → радостная (`CelebAvatar`), см. [[project_pullups_goal_plan]]
