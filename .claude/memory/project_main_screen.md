...
- FAB справа внизу (`components/Fab.tsx`): круглая кнопка с плюсом, по тапу разворачивается dropdown-меню «Создать привычку» (→ `/(tabs)/create-habit`) и «Вступить в группу» (`onPress={() => {}}` — пока заглушка)
...
**How to apply:** Кнопка «Вступить в группу» — следующий шаг (присоединение к групповой привычке по invite-коду через `habits.invite_code`).
