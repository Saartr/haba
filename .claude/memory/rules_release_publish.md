...
Где что лежит на 2026-09-09:
- **в RuStore — versionCode 2** (SDK 55), это версия у пользователей на руках;
- **на сайте `/download/` — versionCode 3** (SDK 57), собрана и выложена, но **в RuStore не отправлялась**
  по решению пользователя: прогон по Android был беглый.

Значит при следующей отправке в стор номер должен быть не меньше 3. И ломающие изменения
проверять до релиза, а не после: у людей уже есть работающая версия.
