...
- `users.tg_id` и ветка скачивания аватара по нему оставлены: у старых аккаунтов данные
  есть, бот жив. Войти через Telegram больше нельзя.
- `app/dev.tsx` (витрина компонентов) всё ещё показывает Telegram-иконку в примерах кнопок —
  это демо, не авторизация.
