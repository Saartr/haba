...
### Уведомления — переключатель в настройках всё ещё disabled
Push-уведомления (FCM) уже реализованы на бэкенде и фронте — см. [[project_push_plan]]. Но тоггл «Уведомления» в `app-settings.tsx` остаётся `disabled` (захардкожен), не связан с реальным включением/выключением push.
**Что нужно:** связать тоггл с реальной подпиской/отпиской от уведомлений (либо удалить тоггл, если такой настройки не предполагается).
