...
**Проверено в консоли 2026-09-08:** полей для redirect URL нет ни на вкладке App, ни на
Authorization (там способы входа и поля регистрации), ни на Access (там данные: почта,
телефон). Платформа Android зафиксирована. Значит **второе приложение обязательно**, и
именно с платформой «Веб-сайт», а не iOS: наш флоу возвращает код на
`https://apptapa.ru/auth/vk/callback`, а платформа iOS потребовала бы кастомной схемы, что
сломало бы проверку в Expo Go (там схема `exp://`).

Код к этому готов: `verifyVkToken` принимает ключи параметром, достаточно задать
`VK_WEB_CLIENT_ID`, `VK_WEB_CLIENT_SECRET`, `VK_WEB_SERVICE_TOKEN` в `.env`.

**Блокирует:** решение пользователя завести второе приложение. Apple-аккаунт НЕ нужен —
