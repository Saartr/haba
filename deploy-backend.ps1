...
ssh.exe Tapa "cd /var/www/haba && git pull --ff-only && echo done_pull"
Write-Host "--- git pull done ---"

ssh.exe Tapa "cd /var/www/haba/backend && npm install --omit=dev"
Write-Host "--- npm install done ---"

ssh.exe Tapa "pm2 restart step-bot"
Write-Host "--- pm2 restarted ---"

ssh.exe Tapa "pm2 list"
